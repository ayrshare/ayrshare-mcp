...
3. **Profile scoping is a connection header, not a tool argument** — none of the tools you use accepts a `profileKey`. (The one MCP tool that takes a target `profileKey` argument, `generate_jwt`, belongs to the profile-manager agent, not this one.) To act as a specific client profile, the MCP connection must carry that profile's `Profile-Key` header (see getting-started). With none set, calls act on the primary profile; if the user has multiple profiles, confirm which one they mean rather than adding a key to tool arguments.
