...
- Generate a social-account linking URL for a profile with `generate_jwt_social_linking_url` (the link a client opens to connect their networks; target the profile via the `Profile-Key` header)
...
| `mcp__ayrshare__generate_jwt_social_linking_url` | Mint a single sign-on linking URL for the profile set by the `Profile-Key` header (the link a client opens to connect their own networks) |
...
2. **Act as the new profile** — set the MCP connection's `Profile-Key` header to that `profileKey` (see getting-started) and restart. `create_profile` and `list_profiles` are account-level (API key only) and ignore this header, but the next step and all downstream post / analytics / history tools act on whichever profile it selects.
3. **Mint the linking URL** — call `mcp__ayrshare__generate_jwt_social_linking_url` (it targets the profile from the `Profile-Key` header you just set; **no `profileKey` argument, and no private key or domain** — the server derives those) to get a hosted linking `url`. Hand it to the client; they open it in a browser to OAuth their accounts. Requires a provisioned social-linking domain (Business/Enterprise). (The dashboard / REST `generateJWT` are equivalent alternatives.)
4. **Verify** — `mcp__ayrshare__list_profiles` shows the profile and its linked platforms.

## Behavioral rules

1. **No tool takes a `profileKey` argument; profile scoping is the `Profile-Key` header.** `create_profile`/`list_profiles` are account-level (Business API key) and ignore `Profile-Key`. `generate_jwt_social_linking_url` is profile-scoped: it **requires** the `Profile-Key` header set to the target sub-profile, and needs no private key or domain (the server derives them from your authenticated account). It also requires the account to have a provisioned social-linking domain (Business/Enterprise). The downstream post / analytics / history tools use the same header.
