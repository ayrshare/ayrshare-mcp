...
- **Attaching media:** put image/video URLs in `mediaUrls`. Verify them first — sequencing in Gotchas, full media workflow in `../media/SKILL.md`.
...
- **Media must exist and verify before you reference it.** A `mediaUrls` entry that is unreachable or the wrong format fails the post. Upload/verify media first via `../media/SKILL.md` (`mcp__ayrshare__verify_media`), then pass the confirmed URLs into `create_post`.
