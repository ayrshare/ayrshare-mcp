...
- **Editing** is primarily for PENDING posts (scheduled, awaiting approval). Use `mcp__ayrshare__update_post` with the returned `id` and at least one editable field: `scheduleDate` (reschedule), `approved` (approve), `notes`, `disableComments`, `scheduledPause`, or `youTubeOptions`. Two `update_post` gotchas worth knowing up front: `youTubeOptions` reuses the create-side schema, so `title` is required even for a visibility-only change (re-send the existing title or the call returns a 400); and `disableComments` on update applies only to Instagram and LinkedIn, where disabling on LinkedIn deletes existing comments.
