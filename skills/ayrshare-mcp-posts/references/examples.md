...
Profile scoping is **not** in these payloads — it is the `Profile-Key` connection header (set in the MCP client config), never a per-call `profileKey` argument. See `../../ayrshare-mcp-getting-started/SKILL.md`.
...
The `mediaUrls` entry should be a URL you've already confirmed reachable/valid via `mcp__ayrshare__validate_media` (see `../../ayrshare-mcp-media/SKILL.md`).
