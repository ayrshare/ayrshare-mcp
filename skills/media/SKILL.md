...
- Media is referenced by URL in a post's `mediaUrls` array — see `../post/SKILL.md`.
...
- **Media must exist and verify before `create_post` references it.** Don't put a not-yet-uploaded or unverified URL in `mediaUrls`. Confirm with upload/verify first, then reference the resulting URL (cross-link: `../post/SKILL.md`).
