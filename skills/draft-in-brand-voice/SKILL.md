...
description: |
  Draft social posts that match a brand's established voice for the Ayrshare MCP server — by first pulling the profile's own past posts (history), reading their tone, format, and what performed, then writing platform-appropriate copy in that voice and validating it before anything is published. Use whenever someone wants on-brand content rather than generic copy: "write a post the way we usually post", "draft this in our brand voice", "match our tone", "write like our LinkedIn", "we have a voice, use it", or any time content is being created for a profile whose established style should be matched instead of invented. Trigger even without the word "Ayrshare" — if the user wants content that sounds like an existing account, this is the skill. It orchestrates `mcp__ayrshare__get_post_history` / `mcp__ayrshare__get_platform_history` (the voice + performance sample), optionally `mcp__ayrshare__get_post_analytics` (what worked), `mcp__ayrshare__generate_post` and/or your own drafting, then `mcp__ayrshare__validate_post` — it produces DRAFTS and never publishes on its own (publishing is the explicit `create_post` step in `../post/SKILL.md`). For the shared auth model and retry rules, see `../getting-started/SKILL.md`.
---
...
4. **(Optional) Find what worked.** If the goal is performance, not just consistency, pull `mcp__ayrshare__get_post_analytics` on a few past posts and weight the draft toward the formats/voice that engaged best. This is the "train on voice + optimize on history" pairing — say so to the user when you use it.
