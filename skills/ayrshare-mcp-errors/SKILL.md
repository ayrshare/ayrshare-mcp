...
This tool is reached over the same authenticated connection as every other Ayrshare MCP tool. Profile context, where relevant, comes from the connection's `Profile-Key` header, not a per-call argument — there is **no** `profileKey` parameter, and `passthrough` cannot carry one (it is a blocked credential key). Full two-layer model: `../ayrshare-mcp-getting-started/SKILL.md`.
...
- **`explain_error` is itself a read.** If the lookup call fails (rare), surface it plainly; don't loop. (Mirrors the global retry-safety rule in `../ayrshare-mcp-getting-started/SKILL.md`.)
