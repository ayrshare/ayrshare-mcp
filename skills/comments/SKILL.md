...
| `mcp__ayrshare__reply_comment` | Reply to an EXISTING comment | POST `/comments/reply` | `commentId` (Ayrshare Comment ID, or Social Comment ID if `searchPlatformId`), `comment`, `platforms` (REPLY_PLATFORMS — bluesky, facebook, instagram, linkedin, tiktok, twitter, youtube; note: no `reddit`; exactly one if `searchPlatformId`) | `searchPlatformId` (bool), `mediaUrls` (1), `videoId` (required for TikTok Social-Comment-ID replies), `objResponse` (bool), `passthrough` |
