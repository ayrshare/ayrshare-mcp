...
All four tools run against the Ayrshare MCP server at `https://us-central1-ayrshare-dev.cloudfunctions.net/mcp` and authenticate with the Business API key (the `Authorization: Bearer ${AYRSHARE_API_KEY}` header). No `profileKey` is used as the auth key. Endpoints below are the underlying Ayrshare REST routes each tool maps to.
