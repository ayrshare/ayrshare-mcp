...
Both tools run against the Ayrshare MCP server at `https://us-central1-ayrshare-dev.cloudfunctions.net/mcp` and authenticate with the Business API key (the `Authorization: Bearer ${AYRSHARE_API_KEY}` header). No `profileKey` is used as the auth key, and **neither tool takes a `profileKey` argument** — to act as a specific profile you set the `Profile-Key` connection header in the MCP client config, not a per-call parameter. Endpoints below are the underlying Ayrshare REST routes each tool maps to.
